...
void LM(int_type* mtx_as_vec, unsigned long long int steps, unsigned long long int steps_remainder, int_type *L1_vector, int *L1_strategy, int iShorter, int iLonger){ // This function calculates the L1 norm.
...
void calc_Lnorm(item_calc* second){
	...
	else if(second->n == 1 && second->marginal == 1){ // If the order of the L norm is 1 and we consider the marginals then this part of the code will be executed.
		#pragma omp parallel num_threads(second->copyNum)
		{
			LM(second->mtx_as_vec, second->steps, second->steps_remainder, Ln_vector, Ln_strategy, second->iRows_reduced, second->iCols_reduced); // The calculation of the L1 norm.
		}
	}
...
void arguments_OMP(item* first, int* num_of_threads, int* argc, char** argv){
	...
	*num_of_threads = t;
	...
	if(*argc < 5 || first->n_original > 1){first->marginal = 0;}
	else if(*argc == 5) {
